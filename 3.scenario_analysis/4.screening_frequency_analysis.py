...
def costs_analysis():
    import pandas as pd
    file_names = [
        ['4.simulation_outputs/1year/costs/costs_0_8.csv', 'Annual'], 
        ['4.simulation_outputs/2year/costs/costs_0_8.csv', 'Biennial'], 
        ['4.simulation_outputs/3year/costs/costs_0_8.csv', 'Triennial'],
        ['4.simulation_outputs/4year/costs/costs_0_8.csv', 'Quadrennial']]
    ...
    base_cost = costs.loc['Annual']['Total Cost M CLP']
    for f in file_names:
    ...
    costs.to_csv('4.simulation_outputs/2year/costs_summary.csv', sep=';', encoding='utf-8', index=True)
    ...
    ax.set_title('Treatments by Screening Frequency')
    ax.invert_yaxis()
    ...
    for i, v in enumerate(costs['AsymptomaticTreatmentsPercentage']):
        ax.text(v, i, '{:.2%}'.format(v), ha='left', va='center', fontsize=9)
    

    plt.savefig('4.analysis_plots/screening_efficacy_by_frequency.png')


    fig, ax = plt.subplots(1, 4, figsize=(12, 7))
    #fig.position = (0, 6)
    fig.tight_layout(rect=[0.05,0.13,0.95,0.9]) 
    adherences = costs.index
    adherences = [x for x in adherences]
    for i in range(4):
        ...
        ax[i].set_ylim(0, 41500)

        # Put the values in the top of the bars as integers
        for p in ax[i].patches:
            ax[i].annotate(str(int(p.get_height())), (p.get_x() * 1 - 0.2, p.get_height() * 1.015), fontsize=9)

        # Add the total number of FIT and Colonoscopy at the bottom of each graph
        ax[i].text(-0.5, -4000, 'FIT: ' + str(format_with_spaces(int(costs.loc[adherence, 'FIT']))), fontsize=10)
        ax[i].text(-0.5, -5500, 'Colonoscopies: ' + str(format_with_spaces(int(costs.loc[adherence, 'Colonoscopy']))), fontsize=10)
    ...
    plt.savefig('4.analysis_plots/treatments_by_frequency.png')
    ...
    plt.savefig('4.analysis_plots/costs_frequency.png', dpi=600)
    #plt.show()
